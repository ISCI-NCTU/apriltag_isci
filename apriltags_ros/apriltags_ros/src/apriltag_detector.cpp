...
AprilTagDetector::AprilTagDetector(ros::NodeHandle& nh, ros::NodeHandle& pnh): it_(nh)
{
    ...
    image_pub_ = it_.advertise(camera_name+"/tag_detections_image", 1);
    detections_pub_ = nh.advertise<AprilTagDetectionArray>(camera_name+"/tag_detections", 1);
    pose_pub_ = nh.advertise<geometry_msgs::PoseArray>(camera_name+"/tag_detections_pose", 1);
    cam_pose_ = nh.advertise<MetaPose>(camera_name+"/cam_pose", 1);
}
